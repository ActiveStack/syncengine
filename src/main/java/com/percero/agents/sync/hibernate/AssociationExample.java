...
public class AssociationExample implements Criterion {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 3332913458038294975L;
...
/**	public static interface PropertySelector {
	...
	static final class AllPropertySelector implements PropertySelector {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -782949744833744192L;
	...
	@SuppressWarnings("serial")
	static final class NotNullOrZeroPropertySelector implements
			PropertySelector {
		public boolean include(Object object, String propertyName, Type type) {
			return object != null
	...
	public static AssociationExample create(Object entity) {
		if (entity == null)
			throw new NullPointerException("null AssociationExample");
	...
	private boolean isPropertyIncluded(Object value, String name, Type type) {
		return !excludedProperties.contains(name)
				...
				&& (!type.isAssociationType() || (type.isAssociationType()
						&& includeAssociations && !type.isCollectionType()));
	}
	...
	public String toSqlString(Criteria criteria, CriteriaQuery criteriaQuery)
		...
		EntityPersister meta = criteriaQuery.getFactory().getEntityPersister(
				criteriaQuery.getEntityName(criteria));
		String[] propertyNames = meta.getPropertyNames();
		Type[] propertyTypes = meta.getPropertyTypes();
		// TODO: get all properties, not just the fetched ones!
		Object[] propertyValues = meta.getPropertyValues(entity, getEntityMode(
		...
		for (int i = 0; i < propertyNames.length; i++) {
			...
			boolean isPropertyIncluded = i != meta.getVersionProperty()
					&& isPropertyIncluded(propertyValue, propertyName,
							propertyTypes[i]);
			if (isPropertyIncluded) {
				if (propertyTypes[i].isComponentType()) {
					appendComponentCondition(propertyName, propertyValue,
							(AbstractComponentType) propertyTypes[i], criteria,
							criteriaQuery, buf);
				} else {
					appendPropertyCondition(propertyName, propertyValue,
	...
	public TypedValue[] getTypedValues(Criteria criteria,
		...
		Type[] propertyTypes = meta.getPropertyTypes();
		// TODO: get all properties, not just the fetched ones!
		Object[] values = meta.getPropertyValues(entity, getEntityMode(
				criteria, criteriaQuery));
		...
		for (int i = 0; i < propertyNames.length; i++) {
			...
			if (isPropertyIncluded) {
				if (propertyTypes[i].isComponentType()) {
					addComponentTypedValues(name, value,
							(AbstractComponentType) type, list, criteria,
							criteriaQuery);
				} else {
	...
	private EntityMode getEntityMode(Criteria criteria,
	...
	protected void addPropertyTypedValue(Object value, Type type, List<Object> list) {
		if (value != null) {
	...
	protected void addComponentTypedValues(String path, Object component,
			AbstractComponentType type, List<Object> list, Criteria criteria,
			CriteriaQuery criteriaQuery) throws HibernateException {

		if (component != null) {
			String[] propertyNames = type.getPropertyNames();
			Type[] subtypes = type.getSubtypes();
			Object[] values = type.getPropertyValues(component, getEntityMode(
					criteria, criteriaQuery));
			for (int i = 0; i < propertyNames.length; i++) {
				Object value = values[i];
	...
	protected void appendPropertyCondition(String propertyName,
		...
		if (propertyValue != null) {
			...
			SimpleExpression se = (isLikeEnabled && isString) ? Restrictions
					.like(propertyName, propertyValue) : Restrictions.eq(
	...
	protected void appendComponentCondition(String path, Object component,
			AbstractComponentType type, Criteria criteria,
			CriteriaQuery criteriaQuery, StringBuffer buf)
			throws HibernateException {
		...
		if (component != null) {
			...
			for (int i = 0; i < propertyNames.length; i++) {
				...
				if (isPropertyIncluded(value, subpath, subtypes[i])) {
					Type subtype = subtypes[i];
					if (subtype.isComponentType()) {
						appendComponentCondition(subpath, value,
								(AbstractComponentType) subtype, criteria,
								criteriaQuery, buf);
					} else {
						appendPropertyCondition(subpath, value, criteria,
								criteriaQuery, buf);
					}
				}
			}
